...
void VulkanEngine::destroy_buffer(const AllocatedBuffer &buffer)
{
    ...
    _gpuResourceAllocator.destroy_buffer(buffer);
}
...
void VulkanEngine::destroy_image(const AllocatedImage &img)
{
    ...
    _gpuResourceAllocator.destroy_image(img);
}
...
AllocatedBuffer VulkanEngine::create_buffer(size_t allocSize, VkBufferUsageFlags usage, VmaMemoryUsage memoryUsage)
{
    ...
    return _gpuResourceAllocator.create_buffer(allocSize, usage, memoryUsage);
}
...
AllocatedImage VulkanEngine::create_image(VkExtent3D size, VkFormat format, VkImageUsageFlags usage, bool mipmapped)
{
    ...
    return _gpuResourceAllocator.create_image(size, format, usage, mipmapped);
}
                                          ...
                                          bool mipmapped)
{
    ...
    //         vkCmdCopyBufferToImage(cmd, uploadbuffer.buffer, new_image.image, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL,
    //         1,
    //                                &copyRegion);
    ...
    return _gpuResourceAllocator.create_image(data, size, format, usage, mipmapped);
}
...
GPUMeshBuffers VulkanEngine::uploadMesh(std::span<uint32_t> indices, std::span<Vertex> vertices)
{
    ...
    // return newSurface;
    return _gpuResourceAllocator.uploadMesh(indices, vertices);
}
